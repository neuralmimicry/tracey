...
impl JailRuntime {
    fn from_config(config: &TraceyBanJailConfig, tracey_ban_cfg: &TraceyBanConfig) -> Option<Self> {
        ...
        for filter_file in &config.filter_files {
            if let Ok((fails, ignores)) = parse_tracey_ban_filter_file(filter_file) {
...
fn compile_regexes(patterns: &[String], label: &str, jail: &str) -> Vec<Regex> {
    ...
    for pattern in patterns {
        let translated = translate_tracey_ban_regex(pattern);
...
async fn run_jail_action(
    ...
    .await
    {
        ...
        Err(_) => {
            tracing::warn!(jail = %jail.name, "tracey_ban action command timed out");
        }
    }
}
...
fn parse_tracey_ban_filter_file(path: &Path) -> std::io::Result<(Vec<String>, Vec<String>)> {
    ...
    for line in raw.lines() {
        ...
        if let Some((key, value)) = trimmed.split_once('=') {
            ...
            if key == "failregex" || key == "ignoreregex" {
                ...
                if !value.is_empty() {
                    if key == "failregex" {
                        fail.push(translate_tracey_ban_regex(value));
                    } else {
                        ignore.push(translate_tracey_ban_regex(value));
                    }
                }
            } else {
                active_key = None;
        ...
        if line.starts_with(' ') || line.starts_with('\t') {
            if let Some(key) = &active_key {
                if key == "failregex" {
                    fail.push(translate_tracey_ban_regex(trimmed));
                } else if key == "ignoreregex" {
                    ignore.push(translate_tracey_ban_regex(trimmed));
                }
            }
        }
    }
...
fn translate_tracey_ban_regex(input: &str) -> String {
